---
title: ANALYSIS OF POPULATION STRUCTURE, TICK RADSEQ DATA
output: pdf_document
---
```{r install-adegenet, eval=FALSE, echo=FALSE}
...
```{r load-pkgs, echo=FALSE, include=FALSE}
library("ape")
library("genetics")
library("pegas")
library("seqinr")
...
source('~/Desktop/UT_ddRADseq/PlotPrevCI.r')
```

# Data by site

```{r load-site, results='hide', tidy=FALSE}
...
This is the data filtered by HB-raccoon, HB-opossum, SRT-raccoon, and SRT-opossum separately using VCF tools, and then merged together on only shared loci in R. This dataset contains `r site@n.loc` SNPs. The @pop slot contains only collection site information.

# Data by site-host

These are the **exact same data as above**, but the @pop slot reflects the host and collection site.

```{r load-site-host, results='hide'}
...
# Data by host, HB only

```{r HB-data, results='hide'}
setwd('~/Dropbox/ddRADseq/Final_Analysis/Structure_by_Host_HB/')
tick.snps2<-read.snp('Final_Pseudoref_minmeanDP20_minGQ25_maf0.05_HB_only_maxmissing0.75_MERGED_FOR_ADEGENET.finalSNPs.snp')
```

```{r include=FALSE, eval=FALSE}
# the dataset with related ticks removed
#setwd('~/Dropbox/ddRADseq/Final_Analysis/Structure_by_Host_HB/')
...
These data were filtered by HB-raccoon and HB-opossum separately using VCFtools, then merged on shared loci in R. This dataset is consequently larger, containing `r tick.snps2@n.loc` SNPs. The @pop slot contains only host information.

# Data by host, SRT only

```{r SRT-data}
setwd('~/Dropbox/ddRADseq/Final_Analysis/Structure_by_Host_SRT/')
tick.snps3<-read.snp('~/Dropbox/ddRADseq/Final_Analysis/Structure_by_Host_SRT/Final_Pseudoref_minmeanDP20_minGQ25_maf0.05_SRT_only_maxmissing0.75_MERGED_FOR_ADEGENET.finalSNPs.snp')
...
# Infer populations from the data using the BIC technique

If there is a strong signal of population structure, we should be able to infer the structure with *adegenet*'s *find.clusters()* function. Note that cluster assignment isn't deterministic -- individuals may be assigned to different clusters each time the code is executed. However, the overall patterns will remain the same.

## Attempt to find 4 populations

If HB-raccoon, HB-opossum, SRT-raccoon, and SRT-opossum are distinct subpopulations, then *find.clusters()* should detect them.

```{r 4-pop}
# run clustering
try.four<-find.clusters(site, n.pca = 33, n.clust = 4)

# reformat clustering into a table showing group assigned and original group
...
ulm.df.try4<-merge(uldf, extra.data, by.x='ID', by.y='combo.label')
try4.results<-cbind(as.character(ulm.df.try4$ID), ulm.df.try4$group, paste(ulm.df.try4$host.species, ulm.df.try4$coll.site, sep='-'))
rt<-table(try4.results[,2], try4.results[,3])
knitr::kable(rt)

par(mfrow=c(1,2), mar=c(3,2,4,2))
barplot(t(rt), legend=c(unique(try4.results[,3])))
table.value(rt, col.labels=c(unique(try4.results[,3])))
```

## Attempt to find 4 hierarchical clusters

This method breaks the data into its assigned subpopulations, in this case HB and SRT. Then clustering is done within those subpopulations. The ordering of the samples in the genid object should be the same as the ordering in *extra.data*. The hierarchical method is not ideal because we can't save enough principle components (n.pca<=N/3 where N is the sample size of each subpopulation).

```{r hier-clust}
hier.clust<-find.clusters(site, clust=site@pop, n.pca=10, n.clust=2)
match<-cbind(as.character(extra.data$host.species), 
             as.character(extra.data$coll.site), as.character(hier.clust$grp))
```

## Attempt to find 3 populations

This one isn't really rooted in a biological hypothesis, so it can be disregarded. Code, for posterity's sake: 

```{r 3-pop}
## 3 populations, infer
try3<-find.clusters(site, n.pca=33, n.clust=3)
...
ulm.df.try3<-merge(uldf3, extra.data, by.x='ID', by.y='combo.label')
...
par(mfrow=c(1,2), mar=c(3,2,4,2))
barplot(t(rt3), legend=c(unique(try3.results[,3])))
table.value(rt3, col.labels=c(unique(try3.results[,3])))
```

## Attempt to find 2 populations

If there is either site-only or host-only structure, looking for 2 populations should find it.

```{r 2-pop}
try2<-find.clusters(site, n.pca=33, n.clust=2)
...
par(mfrow=c(1,2), mar=c(3,2,4,2))
barplot(t(rt2))
legend('topleft', unique(try2.results[,3]), fill=gray.colors(4))
table.value(rt2, col.labels=c(unique(try2.results[,3])))
```

# DAPC by site and host

Since we have *a priori* hypotheses about what the populations should be, we can use that information in a discriminant analysis of principle components (DAPC).
...
compoplot(sh.test, lab='')
```

## n-1 resampling

We can re-do the DAPC inference with all individuals *EXCEPT* one, and see if the DAPC model can predict the proper group membership for the excluded individual.

```{r DAPC-site-host-n-1, cache=TRUE}
# start a counter for correct assignments
tally4<-0
...
# iterate through the individuals, removing one each time, and perform the DAPC
...
overall<-tally4/nInd(site.host)

#names vector is pulled from the snp data, so the order should be preserved
what.worked<-cbind(tick.names.formatted, assign, true, tf) 
...
hbo.pc<-hbo.correct/hbo
hbr.pc<-hbr.correct/hbr
srto.pc<-srto.correct/srto
srtr.pc<-srtr.correct/srtr

```

# DAPC between Sites

None of the above analyses indicated population structure at the site and host level. We can try another approach, to first look for population structure between sites, and then by host within site. This is the first phase of that analysis.

The data have already been read in; object "site" is the one we'll use for this analysis.

```{r exclude-2, eval=FALSE, include=FALSE}

# code for making figures that don't need to be re-made
...
# what is the optimal number of PCs to retain? we keep as many as we can...
par(mfrow=c(1,1))
temp<-optim.a.score(site.dapc)
#... the true optimum may be higher than we can retain...

# a naive approach to predictive re-sampling (don't use)
kept.id<-unlist(tapply(1:nInd(site), pop(site), function(e) sample(e, 5, replace=FALSE))) 
x<-site[kept.id]
x.sup<-site[-kept.id]
...
compoplot(site.dapc, lab=site@pop)
```

### n-1 resampling

```{r DAPC-site-n-1, cache=TRUE}
tally<-0
assign.site<-c()
true.site<-c()
tf.site<-c()
...
accuracy.site<-tally/nInd(site)
```

```{r by-hand-compoplot, eval=FALSE, include=FALSE}

# Before I found the compoplot() function, I did it by hand...
...
So, it looks like there is structure separating the two sites (weak but cannot be ignored in analysis). While hierarchical clustering didn't work very well, perhaps we'll have more power to detect clusters corresponding to host ID within each site if we run the DAPC separately on data by site.

# Data by host, HB only

```{r DAPC-HB, cache=TRUE}
setwd('~/Dropbox/ddRADseq/Final_Analysis/Structure_by_Host_HB/')
dapc2<-dapc(tick.snps2, pop=tick.snps2@pop, n.pca=27, n.da=1) 
scatter.custom(dapc2)
axis(side=2, las=1)
...
```{r by-site-and-HB-plot}
par(mfrow=c(1,2), mar=c(5,4,3,2))
scatter.custom(site.dapc, col=c('gray', 'black'))
mtext(side=3, line=1, text=paste(site@n.loc, 'Loci from Both Sites'))
scatter.custom(dapc2, col=c('gray', 'black'))
mtext(side=3, line=1, text=paste(tick.snps2@n.loc, 'Loci from HB Only'))
...
for(i in 1:nInd(tick.snps2)){
  ...
  x.kp2.dapc<-dapc(x.kp2, n.pca=27, n.da=1)
...
accuracy.hb<-tally2/nInd(tick.snps2)
...
The sample sizes here are crazy low, but the higher SNP yield and the results are illustrative of a larger potential methodological problem -- the interaction between SNP filtering and sample size.

```{r DAPC-SRT, cache=TRUE}
dapc3<-dapc(tick.snps3, pop=tick.snps3@pop, n.pca=5, n.da=1) 
scatter.custom(dapc3)
legend("topright", legend=c('Raccoon', 'Opossum'), fill=c('red', 'blue'), bty='n')
```

# n-1 resampling

```{r DAPC-SRT-n-1, cache=TRUE}
tally.srt<-0
assign.srt<-c()
true.srt<-c()
tf.srt<-c()
...
accuracy.srt<-tally.srt/nInd(tick.snps3)
...
o.srt.correct<-length(subset(what.worked.srt, true.srt=='O' & tf.srt=='1')[,1])
o.srt.n<-length(subset(what.worked.srt, true.srt=='O')[,1])
r.srt.correct<-length(subset(what.worked.srt, true.srt=='R' & tf.srt=='1')[,1])
r.srt.n<-length(subset(what.worked.srt, true.srt=='R')[,1])
o.srt.pc<-o.srt.correct/o.srt.n
r.srt.pc<-r.srt.correct/r.srt.n
```

```{r old-plots, include=FALSE, eval=FALSE}
...
```{r really-old, include=FALSE, eval=FALSE}
...
```{r pca-non-related, include=FALSE, eval=FALSE}
...
by.site.CI<-KL.CI(site.data$percent, site.data$pos, site.data$n, site.data)
barplot2(by.site.CI$percent*100, ylim=c(0,100), plot.ci=TRUE, ci.u=by.site.CI$ci.u*100, ci.l=by.site.CI$ci.l*100, names=c('HB', 'SRT'), las=1, cex.lab=1.5, cex.names=1.5, cex.axis=1.5, ylab=c('Percent Accuracy'), col='slategray2')
...
by.HB.CI<-KL.CI(hb.ro.data$percent, hb.ro.data$pos, hb.ro.data$n, hb.ro.data)
barplot2(by.HB.CI$percent*100, ylim=c(0,100), plot.ci=TRUE, ci.u=by.HB.CI$ci.u*100, ci.l=by.HB.CI$ci.l*100, names=c('Raccoon', 'Opossum'), las=1, cex.lab=1.5, cex.names=1.5, cex.axis=1.5, ylab=c('Percent Accuracy'), col='slategray2')
...
by.SRT.CI<-KL.CI(srt.ro.data$percent, srt.ro.data$pos, srt.ro.data$n, srt.ro.data)
barplot2(by.SRT.CI$percent*100, ylim=c(0,100), plot.ci=TRUE, ci.u=by.SRT.CI$ci.u*100, ci.l=by.SRT.CI$ci.l*100, names=c('Raccoon', 'Opossum'), las=1, cex.lab=1.5, cex.names=1.5, cex.axis=1.5, ylab=c('Percent Accuracy'), col='slategray2')
...
all.CIs<-KL.CI(all.for.CI[,1], all.for.CI[,3], all.for.CI[,2], all.for.CI)

par(mfrow=c(1,1))
barplot2(all.accuracy*100, ylim=c(0,100), plot.ci=TRUE, ci.u=all.CIs[,4]*100, ci.l=all.CIs[,5]*100, names=c('Single Model', 'Site', 'Host | Site 1', 'Host | Site 2'), las=1, cex.lab=1.5, cex.names=1.5, cex.axis=1.5, ylab=c('Percent Accuracy'), col='slategray2')
...
mean.acc<-c(mean(sm.data$percent), mean(site.data$percent), mean(hb.ro.data$percent), mean(srt.ro.data$percent))
...
```{r final-images}
pdf(file='~/Dropbox/ddRADseq/Final_Analysis/SummaryFig.pdf', height=20/2.54, width=20/2.54)
par(mfrow=c(2,2))
scatter.custom(site.dapc, col=c('gray', 'black'), cex.lab=1.5, cex.axes=1.5)
legend('topright', legend=c('Site 1', 'Site 2'), fill=c('gray', 'black'), bty='n')
mtext(side=3, line=1, text=paste(site@n.loc, 'Loci from Both Sites')) 
barplot2(by.site.CI$percent*100, xlim=c(0,2.1), width=c(1), space=c(0.1), ylim=c(0,100), plot.ci=TRUE, ci.u=by.site.CI$ci.u*100, ci.l=by.site.CI$ci.l*100, las=1, cex.lab=1.5, cex.names=1.5, cex.axis=1.5, ylab=c('Percent Accuracy'), col=c('gray', 'gray30'))
axis(side=1, line=1.1, at=c(0.6,1.7), labels=c(paste('Site 1\nn=',by.site.CI$n[1], sep=''), paste('Site 2\nn=',by.site.CI$n[2], sep='')), tick=F)
scatter.custom(dapc2, col=c('gray', 'black'))
legend('topright', legend=c('Opossum', 'Raccoon'), fill=c('gray', 'black'), bty='n')
mtext(side=3, line=1, text=paste(tick.snps2@n.loc, 'Loci from Site 1 Only'))
barplot2(by.HB.CI$percent*100, xlim=c(0,2.1), width=c(1), space=c(0.1), ylim=c(0,100), plot.ci=TRUE, ci.u=by.HB.CI$ci.u*100, ci.l=by.HB.CI$ci.l*100, las=1, cex.lab=1.5, cex.names=1.5, cex.axis=1.5, ylab=c('Percent Accuracy'), col=c('gray', 'gray30'))
axis(side=1, line=1.1, at=c(0.6,1.7), labels=c(paste('Raccoon\nn=',by.HB.CI$n[1], sep=''), paste('Opossum\nn=',by.HB.CI$n[2], sep='')), tick=F)
dev.off()
```

```{r accuracy-plots, include=FALSE, eval=FALSE}
### EEID 2015 plot
...
barplot(c(overall, hbo.pc, hbr.pc, srto.pc, srtr.pc),
        names=c('overall', 'HB-O', 'HB-R', 'SRT-O', 'SRT-R'),
        col=c('gray30', 'blue1', 'blue4', 'red1', 'red4'))
...
```{r exclude-3, include=FALSE, eval=FALSE}
...
# shade polygons with angled lines! so exciting! so unhelpful! save for later!
par(mfrow=c(1,2), mar=c(5,4,3,2))
...
# I am excluding this code for 2 reasons:
# 1. I don't know which data it used; tick.snps isn't defined anywhere
#     The prev. version of this code had it as tick.snps<-read.snp("~/Dropbox/ddRADseq/final_tick_SNPs_no_duplicates.snp")
#     But I don't remember what that data file contains and honestly it's irrelevant because...
# 2. There no reason to expect ticks to cluster by host individual, and the SNPrelate analysis covers this question better anyway
...
dev.off()
```
```{r exclude-4, include=FALSE, eval=FALSE}
# old plot code
...
# oh look! another misguided compoplot!
...
# omitting this for same reasons as HB
